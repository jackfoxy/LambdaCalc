...
Operational Semantics specify how a language behaves. The Lambda Calculus specification, in modified [Baukus-Naur form](https://en.wikipedia.org/wiki/Backus%E2%80%93Naur_form), is from TAPL, p. 72.
...
`E-APP1` and `E-APP2` are congruence rules. `E-APPABS` is the computation or substitution rule, and note the rule matches
when v<sub>2</sub>, which ranges over values, and is the right-hand side of the left-hand side of the rule. Thus this rule 
...
`f: (λf. λx. _)` reads `f` is an abstraction of the form `λf. λx. _`. Note the recursion; and the form of the abstraction body is unimportant.
